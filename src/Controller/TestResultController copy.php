...
class TestResultController extends AbstractController
{
     ...
     */
    public function create(int $id, ValidatorInterface $validator,TestHistoryRepository $testhistoryRepository, Request $request, AddInfection $addInfection, ActionLog $actionLog, DataTableRepository $datatableRepository): Response
    {
        $testresult = new TestResult();

        $form = $this->createForm(TestResultType::class, $testresult);
        $id = $testhistoryRepository->find($id);
        $form->get('testhistory')->setData($id);
